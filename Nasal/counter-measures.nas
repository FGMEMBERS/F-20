...
var flare_releasing_props = ["/f-20/CM/flare-releasing-1",
						      "/f-20/CM/flare-releasing-2",
						      "/f-20/CM/flare-releasing-3",
						      "/f-20/CM/flare-releasing-4"];
...
var dispenser_positions_x = [1.15, 1.3, 1.15, 1.3];
var dispenser_positions_y = [-0.315,-0.315, 0.315, 0.315];
...
{
  var i = 0;
  forindex (i; flare_releasing_props) setprop(flare_releasing_props[i], 0);  
}

#----------------------------------------------------------------------------
var dispensing_flares = false;
  ...
  var i = 0;
   ...
   for (i=0; i<number_of_dispensers; i=i+1)
   {
      ...
      {                
        ...
        setprop(flare_release_x_prop, dispenser_positions_x[i]);
        setprop(flare_release_y_prop, dispenser_positions_y[i]);
        setprop(flare_releasing_props[i], 1); 
        CM_launcher_holes[i][dispenser_count [i]].show();
        flare_released = true;
        updateFlaresCounter();
        dispensing_flares = true;
      }      
    }    
            
    settimer (stopFlares, 0.1);
  if ((number_to_release != 1) and flare_released)
                        repeatFlareDecrease (number_to_release - 1);
  else dispensing_flares = false;                        
}

#----------------------------------------------------------------------------
var releaseFlares = func ()
{
  if (!dispensing_flares)
  {
