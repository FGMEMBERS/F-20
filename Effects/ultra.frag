...
void main (void)
   ...
   /* vec3 lightDir;
    float NdotL;
    vec4 toto_color = fragColor;
    float toto_att, toto_dist;
    
    lightDir = vec3(transformed_position) - vertVec;
     
    toto_dist = length(lightDir);
    ...
    if (NdotL > 0.0) {
        ...
        toto_att = 1.0 / (1.0 +
                     50.0 * toto_dist +
                     15.0 * toto_dist * toto_dist);
        toto_color += toto_att * (light_diffuse * NdotL + light_ambient);
       }

     
       fragColor.rgb = toto_color.rgb;*///max(fragColor.rgb, toto_color.rgb * gl_FrontMaterial.diffuse.rgb * smoothstep(0.0, 1.0, mixedcolor*.5 + toto_color.rgb*.5));
